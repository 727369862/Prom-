...
这个正则看起来就复杂多了，比如获取 statuscode 的地方写的正则是` (?P\d+)`，这个叫做**命名捕获**，核心的正则就是 \d+，但是后面想用这个内容，就给它设置了一个变量叫 code，而 method、url、body_bytes_sent 都是同样的道理。
...
![image-20230714151745113](img/18-06.png)